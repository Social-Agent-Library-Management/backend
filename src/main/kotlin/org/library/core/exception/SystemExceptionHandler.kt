...
class SystemExceptionHandler : ResponseEntityExceptionHandler() {
    ...
    @ExceptionHandler(ExcelImportRejectedException::class)
    fun handleExcelImportRejected(e: ExcelImportRejectedException): ProblemDetail =
        problem(e.errorCode.status, e.errorCode.code, e.errorCode.message).apply {
            setProperty("errorRows", e.errorRows)
